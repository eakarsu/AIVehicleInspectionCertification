...
// Insurance claim automation generating filings from damage photos
// Feature: insurance-claim-automation
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Insurance claim automation generating filings from damage photos.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'insurance-claim-automation', payload, result);
    res.json({ success: true, feature: 'insurance-claim-automation', ...result });
  } catch (err) {
    console.error('insurance-claim-automation analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Insurance claim automation generating filings from damage photos. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'insurance-claim-automation', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'insurance-claim-automation']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'insurance-claim-automation', title: 'Insurance claim automation generating filings from damage photos', project: 'AIVehicleInspectionCertification' });
});
