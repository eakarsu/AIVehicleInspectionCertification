...
// Vision-based damage assessment from photos with repair cost estimates
// Feature: vision-damage-assessment
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIVehicleInspectionCertification" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Vision-based damage assessment from photos with repair cost estimates.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'vision-damage-assessment', payload, result);
    res.json({ success: true, feature: 'vision-damage-assessment', ...result });
  } catch (err) {
    console.error('vision-damage-assessment analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIVehicleInspectionCertification". Return strict JSON only.',
      `Feature: Vision-based damage assessment from photos with repair cost estimates. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'vision-damage-assessment', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'vision-damage-assessment', title: 'Vision-based damage assessment from photos with repair cost estimates', project: 'AIVehicleInspectionCertification' });
});
