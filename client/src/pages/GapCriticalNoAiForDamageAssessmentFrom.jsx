...
export default function GapCriticalNoAiForDamageAssessmentFrom() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-no-ai-for-damage-assessment-from-photos/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-critical-no-ai-for-damage-assessment-from-photos', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-no-ai-for-damage-assessment-from-photos/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Critical: No AI for damage assessment from photos</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AIVehicleInspectionCertification</p>
      </div>
