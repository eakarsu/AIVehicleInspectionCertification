...
export default function GapNoInsuranceClaimIntegrationDirectInsurerApi() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-insurance-claim-integration-direct-insurer-api/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-insurance-claim-integration-direct-insurer-api', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-insurance-claim-integration-direct-insurer-api/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No insurance claim integration / direct insurer API</h1>
