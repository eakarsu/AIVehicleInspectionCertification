...
export default function CfPartsPriceMonitoringWithBulkOrderTiming() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-parts-price-monitoring-with-bulk-order-timing-recommendations/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-parts-price-monitoring-with-bulk-order-timing-recommendations', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-parts-price-monitoring-with-bulk-order-timing-recommendations/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Parts price monitoring with bulk-order timing recommendations</h1>
