...
export default function CfVinDecoderIntegrationForAutoPopulatedVehicle() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-vin-decoder-integration-for-auto-populated-vehicle-profiles/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-vin-decoder-integration-for-auto-populated-vehicle-profiles', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-vin-decoder-integration-for-auto-populated-vehicle-profiles/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">VIN-decoder integration for auto-populated vehicle profiles</h1>
