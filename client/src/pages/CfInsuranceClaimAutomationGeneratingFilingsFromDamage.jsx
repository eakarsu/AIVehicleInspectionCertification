...
export default function CfInsuranceClaimAutomationGeneratingFilingsFromDamage() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-insurance-claim-automation-generating-filings-from-damage-photos/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-insurance-claim-automation-generating-filings-from-damage-photos', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-insurance-claim-automation-generating-filings-from-damage-photos/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Insurance claim automation generating filings from damage photos</h1>
