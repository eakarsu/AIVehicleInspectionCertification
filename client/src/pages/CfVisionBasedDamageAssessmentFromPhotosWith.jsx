...
export default function CfVisionBasedDamageAssessmentFromPhotosWith() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-vision-based-damage-assessment-from-photos-with-repair-cost/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-vision-based-damage-assessment-from-photos-with-repair-cost', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-vision-based-damage-assessment-from-photos-with-repair-cost/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Vision-based damage assessment from photos with repair cost estimates</h1>
