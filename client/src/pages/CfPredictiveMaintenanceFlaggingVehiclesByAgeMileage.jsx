...
// Feature: Predictive maintenance flagging vehicles by age/mileage/condition
...
export default function CfPredictiveMaintenanceFlaggingVehiclesByAgeMileage() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-maintenance-flagging-vehicles-by-age-mileage-condition/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-predictive-maintenance-flagging-vehicles-by-age-mileage-condition', project: 'AIVehicleInspectionCertification' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-predictive-maintenance-flagging-vehicles-by-age-mileage-condition/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Predictive maintenance flagging vehicles by age/mileage/condition</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVehicleInspectionCertification</p>
      </div>
